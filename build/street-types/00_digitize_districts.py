...
def main() -> None:
    ...
    ap.add_argument("--tol", type=float, default=30.0, help="RGB match tolerance")
    ...
    for code, (r, g, b) in pal.items():
        ...
        clean = sieve(raw, size=a.min_px)
        polys = [shape(geom) for geom, val in
                 shapes(clean, mask=clean.astype(bool), transform=transform) if val == 1]
        if not polys:
            print(f"  {code:18} {int(raw.sum()):>7} px matched, all sieved out"); continue
        geom = unary_union(polys)
        feats.append({"district": code, "geometry": geom})
        print(f"  {code:18} {int(raw.sum()):>7} px  ->  {len(polys)} polygon(s)")
    ...
    tag = "  (DRY RUN — pixel space, NOT geographic)" if a.dry_run else ""
